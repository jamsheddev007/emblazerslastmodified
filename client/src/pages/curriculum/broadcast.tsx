...
import { Send, Users, Bell, Megaphone, History, RefreshCw, CheckCircle2 } from "lucide-react";

const sectionOptions = ["A", "B", "C", "D", "E", "F"];

const notificationTypes = [
  { value: "announcement", label: "General Announcement", color: "bg-blue-100 text-blue-800", icon: Megaphone },
  { value: "exam_notice", label: "Exam Notice", color: "bg-purple-100 text-purple-800", icon: Bell },
];
...
export default function CurriculumBroadcast() {
  ...
  const { data: history = [], isLoading: historyLoading, refetch: refetchHistory } = useQuery<any[]>({
    queryKey: ["/api/notifications/general-broadcast-history"],
  });

  const previewMutation = useMutation({
    mutationFn: () => apiRequest("POST", "/api/notifications/general-broadcast-preview", {
  ...
  const sendMutation = useMutation({
    mutationFn: () => apiRequest("POST", "/api/notifications/general-broadcast", {
      type: notifType,
      title: title.trim(),
    ...
    onSuccess: (data) => {
      ...
      queryClient.invalidateQueries({ queryKey: ["/api/notifications/general-broadcast-history"] });
      setTitle("");
      setMessage("");
      setPreview(null);
  ...
  return (
    <ModuleLayout module="curriculum" navItems={curriculumNavItems}>
      <div className="space-y-6">
        <div>
          <h1 className="text-2xl font-bold" data-testid="text-page-title">Parent Broadcast</h1>
          <p className="text-muted-foreground">Send announcements and exam notices to parents</p>
        </div>
                      ...
                      <button
                        ...
                        className={`flex items-center gap-2 px-3 py-2 rounded-lg border text-sm font-medium transition-all ${
                          notifType === t.value
                            ...
                            : "border-border hover:border-indigo-400/50 text-muted-foreground"
                        }`}
                      >
          ...
          <div className="space-y-4">
            ...
            <Card className="bg-muted/40">
              <CardContent className="pt-4 space-y-3">
                ...
                {[
                  "Choose announcement or exam notice type",
                  ...
                  <div key={i} className="flex items-start gap-2 text-xs text-muted-foreground">
                    <div className="w-4 h-4 rounded-full bg-indigo-100 text-indigo-700 dark:bg-indigo-950/50 dark:text-indigo-400 flex items-center justify-center text-[10px] font-bold shrink-0 mt-0.5">{i + 1}</div>
                    {tip}
                  </div>
                ))}
              </CardContent>
